...
public class IseDemo extends Activity implements OnClickListener {
	...
	public void onClick(View view) {
		if(view.getId() == R.id.image_ise_set)
		{
			Intent intent = new Intent(IseDemo.this, IseSettings.class);
			startActivityForResult(intent, REQUEST_CODE_SETTINGS);
		}else if(view.getId() == R.id.ise_start)
		{
				if (mIse == null) {
					return;
				}
		...
		}else if(view.getId() == R.id.ise_parse)
		{
				// 解析最终结果
		...
		}else if(view.getId() == R.id.ise_stop)
		{
				if (mIse.isEvaluating()) {
					mResultEditText.setHint("评测已停止，等待结果中...");
					mIse.stopEvaluating();
				}
		}else if(view.getId() == R.id.ise_cancel)
		{
				mIse.cancel(false);
				mIseStartButton.setEnabled(true);
				mResultEditText.setText("");
