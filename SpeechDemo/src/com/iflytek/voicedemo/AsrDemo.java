...
public class AsrDemo extends Activity implements OnClickListener{
	...
	public void onClick(View view) {	
    	    if(view.getId() == R.id.isr_grammar)
		    {
    	    ...
    	    else if(view.getId() == R.id.isr_lexcion)// 本地-更新词典      注意:更新词典需要在接收到构建语法回调onBuildFinish之后进行，否则会导致错误。
			{ 
    	    ...
    	    else if(view.getId() == R.id.isr_recognize)// 开始识别
			{
    	    ...
    	    else if(view.getId() == R.id.isr_stop)// 停止识别
			{
				mAsr.stopListening();
				showTip("停止识别");
			}
    	    else if(view.getId() == R.id.isr_cancel)// 取消识别
			{
				mAsr.cancel();
				showTip("取消识别");
			}
